...
import me.spec.eris.utils.render.Draw;
...
public class Panel {
    ...
    public void drawScreen(int mouseX, int mouseY) {
    	ClickGUI userInterface = new ClickGUI();
    	
        if (dragging) {
            x = mouseX - (width / 2) + xOffset;
            y = mouseY - (height / 2) + yOffset;
        }
        ClickGUI clickUI = ((ClickGUI)Eris.instance.modules.getModuleByClass(ClickGUI.class));
        GlStateManager.color(1, 1, 1, 1);
        Draw.colorRGBA(ClickGui.getPrimaryColor().getRGB());
        Draw.drawRoundedRect(x, y, width, height, clickUI.corners.getValue());
        GL11.glPushMatrix();
