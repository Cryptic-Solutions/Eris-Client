package me.spec.eris.module.antiflag.prioritization;
...
public class ModulePrioritizer {
	...
	public static boolean isModuleUsable(Module module) {
		...
		Eris.getInstance().modules.getModules().forEach(mod -> 
			{
				if (mod.getModuleType().equals(ModuleType.FLAGGABLE) && !modules.contains(mod) && mod.getModulePriority() != ModulePriority.LOWEST) modules.add(mod);
			}
		);
		if (!modules.isEmpty()) {
			...
			if (modules.get(0) != null) {
				...
				modules.forEach(mod -> {
					...
					if (modules.get(modules.indexOf(mod)) != modules.get(0)) {
						...
						if (modules.get(modules.indexOf(mod)).forcedDelay[modules.indexOf(mod)] > 0) modules.get(modules.indexOf(mod)).forcedDelay[modules.indexOf(mod)]--;
					}
				});
	...
	public static boolean flaggableMovementModules() {
		return Eris.instance.modules.isEnabled(Speed.class) || Eris.instance.modules.isEnabled(Flight.class) || Eris.instance.modules.isEnabled(Longjump.class);
	}
}
