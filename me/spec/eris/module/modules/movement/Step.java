	...
	public void onEvent(Event e) { 
		...
		if (e instanceof EventStep) {
			...
			if (event.isPre()) {
    			...
    			if (stepDelay.hasReached(isInvalid ? 500 : 250)) { 
    				needStep = true;
    			}
				...
				if (height <= 1.) {
					...
					if (first == 0.42f) sendPosition(0,.41999998688698f,0, !(BlockUtils.getBlockAtPos(new BlockPos(posX, posY + 0.41999998688698f, posZ)) instanceof BlockAir), false);
					if (posY + second < posY + height) sendPosition(0,second,0, !(BlockUtils.getBlockAtPos(new BlockPos(posX, posY + second, posZ)) instanceof BlockAir), false);
					return;
